...
const Home = () => {
  ...
  // 3. Sync Logic: Refresh wishlist IDs every time this screen gains focus
  // useFocusEffect(
  //   useCallback(() => {
  //     const loadWishlistPids = async () => {
  //       try {
  ...
  //         }
  //       } catch (e) {
  //         console.error("Error loading wishlist PIDs:", e);
  //       }
  //     };
  //     loadWishlistPids();
  //   }, [])
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get("http://192.168.0.200:8080/api/admin/categories/all");
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get("http://192.168.0.200:8080/api/admin/products/all");
  ...
  const handleToggleWishlist = async (product: Products) => {
    ...
    try {
      ...
      setWishlistPids(updatedWishlist.map(i => i.pid)); 
...
//         await axios.delete(`http://192.168.0.186:8080/api/wishlist/remove/${product.pid}`);
//         setWishlistPids(prev => prev.filter(id => id !== product.pid));
//         if (Platform.OS !== 'web') Alert.alert("Removed", "Removed from wishlist");
//       } else {
//         await axios.post("http://192.168.0.186:8080/api/wishlist/add", { pid: product.pid });
        
//         setWishlistPids(prev => [...prev, product.pid]);
//         if (Platform.OS !== 'web') Alert.alert("Success", "Added to wishlist! ❤️");
//       }
//     } catch (error) {
...
//   };
  const renderProductCard = ({ item }: { item: Products }) => {
    ...
    return (
      <View className="w-44 bg-white rounded-2xl p-3 mr-6 mb-4 border border-gray-100 shadow-sm" style={{ elevation: 3 }}>
        <Image 
        ...
        <View className="flex-row justify-between items-center">
          <View className="flex-row items-center">
            ...
            <Text className="text-xs text-gray-400 line-through ml-3 ">₹{item.actualPrice}</Text>
          </View>
          <Text className="text-[10px] text-green-600 font-bold">{item.discount}% OFF</Text>
        </View>

        <View className="mt-2 flex-row justify-between items-center" >
          <Text numberOfLines={2} className="text-sm font-semibold text-gray-800 h-10">
            {item.pname}
          </Text>
          <Text className="text-[10px] text-gray-600 h-10 mt-2">Stock: {item.stockQuantity}</Text>
        </View>
  ...
  return (
    <View className="flex-1 bg-gray-50">
          ...
          <View className="pt-5 px-5">
            <View className="flex-row justify-between items-center">
              <View className="flex-row items-center">
                ...
                <Text className="text-white text-2xl font-bold ml-3 italic">FastDelivery</Text>
              </View>
              ...
              <View className="flex-row items-center gap-x-2">
                ...
                >
                  <Text className="text-white text-[20px] font-semibold ">Logout</Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        <View className="mt-6">
          <Text className="text-xl font-bold text-gray-800 px-8 mb-4">Categories</Text>
          <FlatList
            data={categories}
            ...
            renderItem={({ item }) => (
              // <TouchableOpacity className="w-[120px] h-[150px] bg-[#fff] p-[10px] rounded-[12px] items-center mr-[15px] shadow-lg m-2">
              //   <Image
    ...
    <Text className="mt-2 text-[15px] font-semibold text-center text-gray-700" >
      {item.cname}
        ...
        <View className="flex-row px-8 mt-8 justify-between">
          <Image 
            source={{ uri: 'https://static.vecteezy.com/system/resources/thumbnails/033/953/976/small_2x/online-shopping-concept-e-commerce-flash-sale-discount-payment-cashless-digital-flat-illustration-the-concept-of-online-shopping-on-social-media-app-shopping-online-on-website-or-mobile-free-vector.jpg' }} 
            className="flex-1 h-96 rounded-2xl mr-8"
            resizeMode="cover"
          />
          <Image 
            source={{ uri: 'https://img.freepik.com/free-vector/delivery-staff-ride-motorcycles-shopping-concept_1150-34879.jpg?semt=ais_hybrid&w=740&q=80' }} 
        ...
        <View className="mt-8 mb-24">
          <Text className="text-xl font-bold text-gray-800 px-5 mb-4">Featured Products</Text>
          <ScrollView 
            horizontal={isWeb} 
